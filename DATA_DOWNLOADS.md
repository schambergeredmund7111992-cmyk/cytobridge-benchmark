...
The paper figures are in [`release_asoc/figures/`](release_asoc/figures/); every
reported number is reproduced by one command (no downloads beyond this
repository):
...
`reproduce.py` reads the frozen split and the stored predictions and prints a
PAPER vs RECOMPUTED verdict per row; the per-cell treated/control counts and
the gene order it needs are shipped inside `release_asoc/`.
