...
def match_drugs(names, smiles_map_norm):
    ...
    'Tofacitinib' -> 'Tofacitinib (CP-690550) Citrate'. Returns (matched, missing)."""
    matched, missing = {}, []
...
def run_real(args):
    ...
    scored_matched, scored_missing = match_drugs(SCORED_9, smap)
    if scored_missing:
    ...
    if args.background_csv:
        ...
        bg_matched, _ = match_drugs(bg_names, smap)
    else:
        # background = ALL sci-Plex drugs with SMILES (documented fallback)
    ...
    bg_matched = {n: s for n, s in bg_matched.items() if _norm(n) not in {_norm(x) for x in scored_matched}}
